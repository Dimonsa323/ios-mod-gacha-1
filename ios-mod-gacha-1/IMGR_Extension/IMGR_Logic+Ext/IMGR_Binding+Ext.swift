...
typealias IMGR_Binding = Binding

extension IMGR_Binding where Value == Bool {
        ...
        self.init(
            ...
            set: { newValue in
                if !newValue {
                    ...
                    Logger.error((error, String(describing: bindingToOptional.wrappedValue)))
                }
            }
        )
    }
